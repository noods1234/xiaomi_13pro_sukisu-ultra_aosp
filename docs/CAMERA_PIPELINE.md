...
| 5 | Kernel/V4L2 direct sensor access | Raw sensor register-level control | **Partially unblocked under Path A (corrected A-3):** `LineageOS/android_kernel_xiaomi_sm8550-modules` contains the Qualcomm **camera-kernel techpack source** — the kernel side of the camera stack builds from source in the full-ROM tree, so kernel-level instrumentation/logging/patching of the camera driver is now possible. The userspace ISP (camx/chi-cdk HAL) remains a proprietary blob from TheMuppets, so this is driver-level access, not full-pipeline control. High skill/effort; research track, not MVP. | High | Fall back to Tier 1/2 |
| 6 | Experimental direct RAW pipeline | True sensor RAW/CinemaDNG | **Research-possible under Path A** via Tier-5 driver source + Camera2 `RAW_SENSOR` (LineageOS's standard Camera2 stack is typically less restrictive than HyperOS's about exposing RAW — verify with the capability dump on the Lineage build, C-3) | Very high | Fall back to DNG stills / high-bitrate log |
